...
class GreedyFullTaskPolicy(Agent):
    ...
    def _choose_target(self, state) -> tuple[int, int] | None:
        ...
        if held is not None:
            if held.name == "soup":
                return self._nearest_reachable(player.position, mdp.get_serving_locations())

            if held.name == "dish":
                ready_pots = list(pot_states.get("ready", []))
                if ready_pots:
                    return self._nearest_reachable(player.position, ready_pots)
                ...
                if almost_ready:
                    return self._nearest_reachable(player.position, almost_ready)
                return None

            if held.name in {"onion", "tomato"}:
                return self._nearest_reachable(player.position, self._pots_that_can_accept_ingredients(state, pot_states))
        ...
        if ready_pots:
            ...
            if counter_dishes:
                return self._nearest_reachable(player.position, counter_dishes)
            dish_disps = mdp.get_dish_dispenser_locations()
            if dish_disps:
                return self._nearest_reachable(player.position, dish_disps)
        ...
        if pots_needing_items:
            ...
            if counter_ingredients:
                return self._nearest_reachable(player.position, counter_ingredients)
            ingredient_disps = self._ingredient_dispenser_locations()
            if ingredient_disps:
                return self._nearest_reachable(player.position, ingredient_disps)
        ...
        if full_not_cooking:
            return self._nearest_reachable(player.position, full_not_cooking)
    ...
    def _nearest_reachable(
        ...
        for pos in positions:
            goals = [p for p in self._adjacent_positions(pos) if p in valid_positions]
            if not goals:
                continue
            path = self._bfs_shortest_path(origin, set(goals), valid_positions, blocked=set())
            if path is not None:
                reachable_positions.append((pos, len(path)))

        if not reachable_positions:
            return self._nearest(origin, positions)

        return min(reachable_positions, key=lambda x: x[1])[0]
