...
class DemonstrationRecorder:
    ...
    def _resolve_output_paths(self, metadata: dict[str, Any]):
        ...
        agent_type = _agent_type_from_metadata(metadata)
        target_dir = self.output_dir / layout_label / agent_type
...
def _agent_type_from_metadata(metadata: dict[str, Any]) -> str:
    policies = metadata.get("policies", {}) or {}
    for agent_id, policy_cfg in policies.items():
        name = str(policy_cfg.get("name", "")).lower()
        if name and name != "human_keyboard":
